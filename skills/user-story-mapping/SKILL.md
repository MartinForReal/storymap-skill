...
description: Run user story mapping (Jeff Patton style) — turns a vague goal, brief, or messy backlog into a sliced delivery plan with per-persona stories, the team's test-playbook blueprint (acceptance criteria + E2E contract). Mines context first (README, code, tests, tracker via MCP), reusing the tracker's taxonomy. Produces a design doc, story map (Markdown + CSV, Mermaid when no tracker defined), prioritized backlog (WSJF/RICE/MoSCoW), optional UX/architect role hints. Auto-activates in a sister framework's Plan stage — Superpowers between `brainstorming` and `writing-plans`/plan generation, gstack `/office-hours`/`/autoplan`/`/plan-*-review` (incl. `/plan-dev-design-review`), or GSD `/gsd discuss`/`/gsd plan-milestone`/Brief/Roadmap authoring — or when a design doc exists and the plan is adjusted. Use when shaping work (discovery, MVP scoping, "what to build first", PI planning, backlog organizing, "stories per persona"). Skip pure engineering — bug fixes, refactors, code review, deploy debugging.
license: MIT
