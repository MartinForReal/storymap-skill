...
CHANGELOG.md               Keep-a-Changelog + SemVer. Currently 0.0.5 (unreleased). Hand-maintained; NOT read by CI.
...
- **Tiered output set:** `design.md` + `storymap.md` + `storymap.csv` (the flat items+status list, parser-derived from `storymap.md`) are **always** produced. `storymap.mmd`, `backlog.md`, `backlog.csv` are produced **only when no issue tracker is defined** (the conditional-artifact rule); when an issue tracker is the system of record the opt-in write-back sets each item's burn-down fields (points + sprint + status) instead. Optional: `handoff.md`, `role-hints.md`, `slice-1-acceptance-criteria.md`, `e2e-test-contract.md`, `breach-decisions.md` (iteration/breach), `tracker-status-update.<ext>` (Step 0.5/6).
...
- **Changing the always/conditional output tiers or CSV headers** → update `grade_runs.py` `REQUIRED_FILES_CANONICAL` (always = `design.md` + `storymap.md` + `storymap.csv`) and keep the conditional-artifact graders (`grade_mermaid`, `grade_method_columns`) tolerant-when-absent for the no-tracker-only files (`storymap.mmd`, `backlog.{md,csv}`), plus `grade_csv_header`'s accepted-headers list (7-col legacy + 9-col current; `storymap.csv` is REQUIRED), per-eval branches reading specific filenames, AND `SKILL.md` § What it produces + `output-routing.md` § What each branch produces + `assets/` + the examples.
...
- **The conditional data files are tracker-gated** — `storymap.mmd`, `backlog.md`, `backlog.csv` are generated only when no issue tracker is defined (when a tracker is the system of record, the burn-down write-back populates the tracker instead — `work-item-tracking.md` § Enable the tracker burn-down). `storymap.csv` is **always** produced (the flat items+status manifest) — don't move it back to the conditional set. The grader reflects this: `REQUIRED_FILES_CANONICAL` = `design.md` + `storymap.md` + `storymap.csv`; `grade_mermaid` / `grade_method_columns` pass when their file is absent.
